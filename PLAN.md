...
1. 🟡 **Move drivers to userspace libOS** — original plan's biggest deviation; XHCI/HDA/NVMe/PCI/ACPI/GOP all still run in-kernel. The foundational IPC/MMIO-passthrough syscall layer this needs first now exists and is proven working; no existing kernel driver has been migrated yet (see below for exactly what remains).
    ...
    - **What this does NOT prove, and what's still needed before any real driver migrates:** `run_elf`/`exec()` is still fully synchronous — a userspace program blocks the entire kernel until it exits (see `process.rs`). A driver needs to run *continuously*, handling interrupts and polling hardware while the rest of the OS keeps going, which needs real concurrent processes — i.e. item #4 below (dynamic task spawn/exit + blocking primitives), completely unimplemented. There's also no permission/capability scoping yet (any process that can reach these syscalls can touch any physical address or I/O port — fine for a single proof-of-concept program, not fine once more than one process exists) and no IRQ-delivery-to-userspace syscall (a real driver needs to block waiting for its device's interrupt rather than busy-poll). None of that is done here — this item lays the hardware-access foundation only.
...
5. 🟡 **USB HID keyboard** — driver support is done in `xhci.rs`; not attached in the default dev QEMU scripts.
