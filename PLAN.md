...
- **Drag-and-drop between (or within) Files windows**: `main.rs`'s `FILE_DRAG: Mutex<Option<FileDrag>>` — armed on the same mousedown that already selects/opens/renames a row (a drag doesn't replace that click action, it's a layered extra, same tradeoff the taskbar's drag-to-reorder made), tracks whether the cursor moved past an 8px threshold, and on release, if it moved, drops the entry into whatever directory the topmost Files window under the cursor is currently browsing (the same window or a different one — dropping on a *specific* subdirectory row to go one level deeper isn't implemented, just "drop anywhere in a Files window's content area"). The actual move is `hepfs::move_entry()` (new): re-parents the `DirEntry` from one directory to another without touching the inode or its contents/data blocks — same "just rewrite the directory entry" approach `rename()` already used, just changing parent instead of name. No cycle detection (moving a directory into its own descendant isn't guarded against) — not reachable through this UI today since a directory's own descendants never appear in its own pane, but worth adding real protection before anything ever calls `move_entry()` with arbitrary caller-supplied paths (e.g. a future `mv` shell command).
...
- **Still a follow-up, not done here**: the large-icon-grid visual reskin (currently a two-pane tree+list, now with real per-type icons — see above) and mouse-wheel scrolling sized to the window; Shift+click/marquee multi-select was added to the desktop (see "Desktop Icons" above) but not to the file manager, which is still single-selection only — see Next Steps.
...
9. **Real desktop shell, remaining pieces** — desktop icons are real/draggable(grid-snapped)/renamable/file-backed and actually open on double-click (files by type, directories as a new Files window); taskbar+Start-Menu pin/unpin (+ taskbar drag-to-reorder, pinned/minimized/running now visually distinct), real pixel-art icon glyphs everywhere (not flat colors), file-manager select/rename-matching-desktop, and drag-and-drop between/within Files windows are all done (see "Desktop Icons"/"Taskbar & Start Menu"/"HepFS File Manager" above). What's left of that same original request:
    ...
    - Shift+click/rubber-band multi-select was added to the desktop (see "Desktop Icons" above) but not to the file manager — its `nav.selected` is still a single `(pane, row)`, not a set, so it's single-selection only.
