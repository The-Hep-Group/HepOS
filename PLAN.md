...
**Recently completed:** **Dynamic task spawn/exit + blocking primitives** — `scheduler.rs`'s `Task`/`Scheduler` used to only ever run the 2 hardcoded boot tasks (`idle`/`blink`); there was no way to spawn a new task at runtime, no exit path, no blocking primitive. Added `scheduler::spawn(name, entry)` (allocates a fresh stack + `Task`, reuses a `Dead` task's slot if one exists — same "evict an exited entry" pattern `process::exec()`'s process table already used, so the task table doesn't grow unbounded across many spawn/exit cycles — otherwise appends a new slot), `scheduler::exit_current()` (marks the calling task `Dead` and switches away, never returning; halts the CPU if it was the last runnable task), and `scheduler::sleep_ms(ms)` (a real blocking primitive: marks the caller `Blocked` with a wake tick computed from `TICK_COUNT`, switches away, and `Scheduler::next()`'s scan promotes `Blocked` → `Ready` once that tick has passed — falls through without blocking if no other task is currently runnable, since a single-task system would otherwise sleep forever with nothing left to wake it). The one subtle fix this needed in `next()`: the outgoing task used to be unconditionally reset to `Ready` on every switch (correct for ordinary timer preemption) — now it's only reset to `Ready` if it was still `Running`, since `exit_current()`/`sleep_ms()` set a different terminal state on themselves *before* yielding, and the old unconditional reset would have silently undone that. Verified with a temporary boot-time test: a spawned task looped 3× incrementing a counter with `sleep_ms(50)` between each (confirmed `counter=3`, i.e. real blocking/resume worked, not a busy-loop), then spawned a second task and exited; the second task spawned a third, and the task table's total slot count stayed the same across all three spawns (confirming dead-slot reuse, not unbounded growth) — full boot regression clean afterward, test code removed.
