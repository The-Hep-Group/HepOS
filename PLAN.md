...
| 🟡 | virtio-gpu driver — `virtio_gpu.rs`: modern virtio-pci transport from scratch (PCI cap-list walk, virtqueue, polled), `GET_DISPLAY_INFO` + full 2D resource create/attach/scanout/transfer/flush pipeline, all verified. Runs as an independent PCI device; **not wired into the real display path** — see Next Steps #18. |
...
18. 🟡 **virtio-gpu driver** — the driver itself is done and verified (2D mode only, no 3D/virgl); **not wired into the real display path** (that's a separate, riskier follow-up — see below). `virtio_gpu.rs`: virtio-gpu has no legacy/transitional PCI ID (unlike e.g. virtio-net), so this implements the *modern* virtio-pci transport from scratch — PCI capability-list walking (`pci::config_read8`, new this session) to locate the common-config/notify-config/device-config regions across whichever BARs the device places them in, the standard virtio init handshake (ACKNOWLEDGE → DRIVER → negotiate `VIRTIO_F_VERSION_1` only → FEATURES_OK → DRIVER_OK), and one split virtqueue (controlq) driven fully synchronously — submit a 2-descriptor chain (request + response), notify, poll the used ring to completion, one command in flight at a time. On top of that, the actual virtio-gpu 2D protocol: `GET_DISPLAY_INFO` (the simplest possible round trip, good for proving the whole transport works end to end) and a full `RESOURCE_CREATE_2D` → `RESOURCE_ATTACH_BACKING` → `SET_SCANOUT` → `TRANSFER_TO_HOST_2D` → `RESOURCE_FLUSH` pipeline. Every wait loop bounded/fails-soft, same lesson as the NVMe/AHCI drivers. Added as an independent PCI device (`-device virtio-gpu-pci` in the QEMU scripts) alongside the existing `-vga std` display, so none of this touches the critical boot path. Verified with a temporary boot-time test: `GET_DISPLAY_INFO` returned a real 1280×800, and a solid-fill test resource round-tripped through the full create/attach/scanout/transfer/flush pipeline successfully — both confirmed via virtio-gpu's own response codes. **Remaining work, not started:** actually switching the kernel's real rendering path over to virtio-gpu (today `framebuffer.rs` still draws every pixel via the CPU into a PMM-backed backbuffer and copies it to HepBL's GOP-provided linear framebuffer) is a materially bigger, separate decision — it means the *boot* display itself would depend on this driver initializing successfully, which is a much higher-stakes change than adding a parallel, optional device.
