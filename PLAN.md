...
> **Last updated:** 2026-07-09 (NVMe — the actual boot disk — migrated to userspace too, 5th of 6 drivers; a hybrid direct-path fallback covers early-boot I/O before the scheduler exists, and `service stop`/`start` is a genuine two-way handoff since disk I/O can't just go away)

---

## Overview

HepOS is a custom x86\_64 operating system written in Rust using an **exokernel architecture**. The kernel does hardware multiplexing plus the delicate one-time bring-up for each device; the ongoing hot-path work for 5 drivers (RTL8139, Intel HDA, AHCI, XHCI, NVMe) now runs as persistent userspace processes talking to the kernel through shared-memory mailboxes — PCI, ACPI, and GOP still run entirely in-kernel. Single user, no permissions, networking partially implemented.
...
| NVMe (storage) | ✅ | The actual HepFS boot disk. One-time bring-up (PCI enable, MMIO map, admin queue, Identify, I/O queue creation) stays in-kernel; ongoing I/O-queue reads/writes now run in userspace (`userspace/nvmed`) — see the userspace-drivers row below. Early boot I/O (before the scheduler exists) uses a preserved in-kernel direct path. |
...
| **Drivers live in userspace libOS** | 🟡 | **5 of 6 now migrated.** RTL8139 (NIC), Intel HDA (audio), AHCI (SATA), XHCI (USB HID), and NVMe (the actual boot disk) all run their ongoing per-request/per-event hot-path work as persistent userspace processes (`userspace/{rtl8139d,hdad,ahcid,xhcid,nvmed}`) talking to the kernel through shared-memory mailboxes — the kernel keeps only the one-time hardware bring-up (PCI enable, DMA buffer allocation, etc.) that needs `pmm`/PCI access no ring-3 process has. A `service`/`kill` terminal command layer manages all 5 (start/stop/enable/disable — see "Service management" above; NVMe's stop/start is a genuine two-way handoff, not a simple one-way trip, since disk I/O can't just stop while the service is down — see the NVMe migration writeup below for why). PCI, ACPI, and GOP still run entirely in the kernel; GOP is the only meaningfully-remaining candidate (PCI/ACPI are one-shot work, not really in scope for this kind of migration). |
...
- **A real bug found via real usage (not synthetic testing) once this shipped**: after `kill`ing `xhcid` and starting it again, the mouse cursor reappeared (the process was alive and had re-mapped everything successfully) but never moved again. Root cause: `xhcid`'s live ring-position state — the event ring's consumer position (`evt_i`/`evt_c`) and each HID device's transfer-ring producer position (`mouse.hid_i`/`hid_c`, and `kbd`'s) — was only ever *written* into the mailbox once, by the kernel's own `init()`, at boot. The running driver kept these as purely local variables, continuously advancing them as it processed real events, but never wrote the updated values back. A freshly-restarted `xhcid` re-reads the mailbox at startup and resumes from those stale *boot-time* positions instead of wherever the killed instance actually left off — desyncing it from both the real xHC hardware (which has long since moved its own internal Event Ring Dequeue Pointer far past that stale index) and the HID ring's actual free/in-use slots. `dequeue()`'s cycle-bit check (`if (trb[3] & 1) != *evt_c as u32`) then never matches what the hardware is really producing, so no report ever reaches the mailbox's report ring again — exactly the "cursor exists, never moves" symptom, and a mode `service stop`/`start`'s own tests hadn't exercised (they checked the *process* restarted and was `running`, not that its *hardware I/O* still worked afterward). Fixed by having `xhcid` write `evt_i`/`evt_c`/`mouse.hid_i`/`hid_c`/`kbd.hid_i`/`hid_c` back into the mailbox once per loop iteration (cheap — a few volatile writes into already-mapped memory, no syscall) — a subsequent restart now correctly resumes from wherever the previous instance actually was, not from boot time. Verified via QEMU-monitor-injected synthetic mouse events (`mouse_move` over the temporary `-monitor telnet:...` flag): confirmed real cursor movement continued to register correctly after a genuine `stop_service()` + `start_service()` cycle, where before the fix it would have gone silent. Worth noting for the *next* stop/restart-capable service, if one's ever added: any driver that keeps its own live protocol/ring state purely in local variables needs to think about what a restart resumes from — this class of bug won't show up in a synthetic "does start/stop work" test, only in one that exercises the actual hardware I/O afterward.
- **Checked every other driver for the same class of bug, and found one more.** Following the `xhcid` fix above, went back through `rtl8139d`, `hdad`, and `ahcid`'s own local state to see which of them had something analogous.
  - **`rtl8139d` had the same bug, fixed a different way**: `rx_off` (the driver's own read position into the 64KB RX ring) started at a hardcoded `0` on every launch. Unlike `xhcid`'s event ring, though, RTL8139's hardware exposes the equivalent position as a genuine, persistent **hardware register** (`CAPR`) that the driver itself already writes after every packet (`sys_port_out(io + CAPR, ...)`) — so instead of adding a mailbox round-trip, the fix reads `CAPR` back at startup and derives `rx_off = (capr + 16) % 65536` from it, recovering exactly where the previous instance left off with no IPC changes needed at all. Verified: after a `stop_service()` + `start_service()` cycle, the new instance's startup `rx_off` matched byte-for-byte where the old instance's last packet had left `CAPR` (confirmed via temporary diagnostics printing both), and a real `ping 10.0.2.2` afterward got a genuine reply. **A residual timing caveat, not a correctness bug**: the first 1-2 ping attempts *immediately* after a restart sometimes hit `net.rs`'s tight 250ms deadline (real settling latency — fresh `SYS_MMAP_MMIO` calls, the driver's own `SYS_WAIT_IRQ` cadence re-establishing itself — occasionally exceeds it), but every attempt after that consistently succeeded; this matches the exact same "unbounded time to resolve without something slowing the loop down" sensitivity already documented for the original RTL8139 migration above, now just re-triggered by a restart instead of only by initial boot.
  - **`hdad` and `ahcid` don't have this bug** — confirmed by inspection, not just assumption. `hdad`'s cooperative-stop handler (added when the `stop` mailbox flag was built) already halts any in-progress DMA stream and zeroes `mb.is_playing` *before* exiting, so there's never any "in-flight" playback state a restart could resume incorrectly — it always restarts into a clean idle state, matching reality. `ahcid` has no persistent ring position at all: every read/write request is fully self-contained (LBA/count/data all freshly supplied via the mailbox each time), and the single AHCI command slot is completely reprogrammed (FIS + PRDT) from scratch on every request — there's nothing left over from a previous instance to desync from.
...
| ✓ | NVMe — admin + IO queues; ongoing I/O-queue reads/writes run in userspace (`userspace/nvmed`) — see the userspace-drivers row below |
...
| 🟡 | Drivers moved to userspace libOS — **5 of 6 done.** RTL8139 (`userspace/rtl8139d`), Intel HDA (`userspace/hdad`), AHCI (`userspace/ahcid`), XHCI (`userspace/xhcid`), and NVMe (`userspace/nvmed`, the actual HepFS boot disk) all run their ongoing hot-path work as persistent userspace processes, talking to the kernel through shared-memory mailboxes (one-time hardware bring-up stays in-kernel, since it needs `pmm`/PCI access no ring-3 process has). A `service`/`kill` terminal command layer (see Terminal → Service management) manages start/stop/enable/disable for all 5. PCI, ACPI, and GOP are still in-kernel; GOP is the only meaningfully-remaining, higher-stakes migration candidate (PCI/ACPI are one-shot enumeration work, not really in scope for this). |
...
1. 🟡 **Move drivers to userspace libOS** — original plan's biggest deviation. **5 of 6 drivers now migrated**: RTL8139 (NIC), Intel HDA (audio), AHCI (SATA), XHCI (USB HID), and NVMe (the actual HepFS boot disk) all run their ongoing hot-path work as persistent userspace processes (`userspace/{rtl8139d,hdad,ahcid,xhcid,nvmed}`), talking to the kernel through shared-memory mailboxes, with a `service`/`kill` command layer to start/stop/enable/disable each one (see the "Service management" section above). PCI enumeration, ACPI, and GOP (the framebuffer) still run entirely in-kernel — PCI/ACPI are mostly one-shot enumeration work with no real ongoing hot path to migrate, so GOP is the one meaningfully-remaining, higher-stakes candidate left (see below for exactly what's left).
    ...
    - **XHCI (USB HID) migrated to userspace too — the fourth real driver migration, and the first genuinely higher-stakes one (a bug here breaks all mouse/keyboard input, not just an unused code path).** `kernel/src/xhci.rs`'s one-time bring-up (HC reset, port power/reset, and the Enable-Slot/Address-Device/Configure-Endpoint command sequence for the mouse and optional keyboard) stays in the kernel — every step there is a synchronous command/wait-for-completion exchange only ever run once per device at boot, needing `pmm`/PCI access no ring-3 process has. The *ongoing* work — draining the event ring for completed HID interrupt-IN transfers and re-queuing the next one — now runs in a new persistent process, `userspace/xhcid`, reusing RTL8139's async fire-and-forget mailbox shape (not AHCI's synchronous request/response one): `xhcid` copies each completed report's raw 8 bytes into a small ring in the mailbox and moves on; the kernel drains that ring once per frame (`poll_mouse()`, called from the same place it always was). Critically, **the actual HID→PS/2/mouse-position translation logic (`handle_mouse_report()`/`handle_kbd_report()`) never moved and needed zero changes** — it's still fed raw bytes, just sourced from the mailbox instead of read directly off `hid_buf_v`, so `main.rs`'s only caller (`xhci::poll_mouse(fb_w, fb_h)`) needed zero changes either.
      ...
      - **What's still not done (before NVMe below):** NVMe (the actual HepFS boot disk), PCI/ACPI (one-shot enumeration/shutdown, not really "drivers" with an ongoing hot path to migrate), and GOP (the framebuffer). NVMe and GOP remain the two highest-stakes migrations left — a bug in either breaks the desktop's basic usability (no real filesystem, or no display at all) rather than one feature.
    - **NVMe migrated to userspace too — the fifth real driver migration, and the highest-stakes one: this is the actual HepFS boot disk.** `kernel/src/nvme.rs`'s one-time bring-up (PCI enable, MMIO mapping, controller disable/enable, admin queue setup, Identify Controller/Namespace, I/O queue *creation*) stays entirely in the kernel and is untouched by this migration — all of it runs on the *admin* queue. The ongoing work — actually issuing reads/writes on the *I/O* queue — moves to a new persistent process, `userspace/nvmed`, reusing AHCI's synchronous request/response mailbox shape.
      - **A constraint none of the previous 4 migrations had**: `read_blocks()`/`write_blocks()` are called synchronously during early boot (HepFS format/mount, `kernel.txt`/demo file writes, desktop icon sync) — all *before the scheduler itself exists*, let alone a userspace process. `nvmed` can only launch from `task_blink`'s own loop, same as every other driver, which is necessarily after all of that early I/O already happened. Solved by keeping the *original*, fully-synchronous in-kernel I/O-queue implementation as `read_blocks_direct()`/`write_blocks_direct()` (unchanged) — early boot uses it directly, since there's no other option — and having the public `read_blocks()`/`write_blocks()` (same signatures, zero caller changes across ~15 call sites in `main.rs`/`hepfs.rs`) route through it whenever `nvmed` isn't the current authority, and through the mailbox once it is.
      - **A real, previously-latent design mistake found via actual testing, not by inspection — and the reason `nvmed`'s stop/start isn't a simple one-way trip like the other 3 services.** The first version of this migration treated `stop_service()` like every other driver's: once handed off, `stop` disabled I/O until `start` relaunched it. That's tolerable for a mouse, some audio, or a disk HepFS doesn't even mount — it is not tolerable for the actual boot filesystem, which something on the desktop touches constantly (icon refresh, file listings) whether or not a human just ran `service stop nvmed`. The very first real stop/start test — background desktop activity, not even the test itself — hit `read_blocks()`/`write_blocks()` while `nvmed` was down, spin-waited forever for a mailbox response that would never arrive, and **panicked the whole kernel** via the same `assert!` that already fired on a genuine unrecoverable disk error (not a new assert — but this migration made "the service is stopped," an ordinary user-reachable state, trigger the identical fatal path as real hardware failure). Fixed by making stop/start a genuine **two-way handoff**: `handoff_from_nvmed()` (called by `stop_service()`, after confirming `nvmed` actually exited) syncs the in-kernel direct path's software queue position (`self.io.sq_tail`/`cq_head`/`phase`) *back* from the mailbox before clearing the "mailbox active" flag — so the direct path picks up exactly where `nvmed` left off and the filesystem stays fully functional the entire time the service is down, only the userspace process itself goes away. `handoff_to_nvmed()` (shared by the very first `spawn_pending_driver()` launch and every subsequent `start_service()`) does the mirror image: builds the mailbox from `self.io`'s *current* live position — always correct, since the direct path was the sole authority for however long `nvmed` was stopped. Re-verified after the fix: a real stop/start cycle with background I/O landing in between shows the second `nvmed` launch picking up a visibly *advanced* `sq_tail`/`cq_head` from the first (`14/14` → `40/40` in one test run, proving the direct path really did handle real I/O correctly during the gap, and the restart really did resume from the right place, not a stale snapshot) — zero panics across repeated cycles.
      - **A second real bug, same class as AHCI's**: `Mailbox`'s header fields plus its 4096-byte `data` buffer (sized to exactly match `hepfs::BLOCK_SIZE`, same reasoning as `ahci.rs`'s identical buffer) add up to just over one page (4168 bytes) — `nvmed`'s very first boot test failed with `"failed to map mailbox"` because the kernel had only granted/allocated one page. Fixed with `pmm::alloc_contiguous(2)` and a compile-time `assert!(size_of::<Mailbox>() <= MAILBOX_PAGES * 4096)` guard, identical to AHCI's fix for the identical mistake.
      - **Built in from the start, not discovered the hard way again**: NVMe's SQ/CQ doorbell registers are write-only per spec (unlike RTL8139's readable `CAPR`), so there's no hardware register `nvmed` could recover `sq_tail`/`cq_head`/`phase` from after a restart — it persists these into the mailbox every loop iteration itself, the same fix `xhcid`'s event/HID rings needed after the fact.
      - **Also hardened the handoff itself against a launch that doesn't land**: `handoff_to_nvmed()` never marks the mailbox "active" (never lets `read_blocks()`/`write_blocks()` route through it) until it has actually spin-waited for `nvmed` to register as running — an earlier version finalized the handoff the instant the launch was merely *queued*, so the mailbox-too-small bug above (or any future launch failure) would have left every subsequent I/O call spinning forever against a mailbox nobody was servicing, then panicking exactly like the stop/start bug above. Never finalizing early means a failed handoff simply falls back to the always-working direct path instead.
      - Verified end-to-end: all early-boot self-tests (`NVMe R/W OK`, HepFS format, `Read back: Hello from HepOS!` byte-exact round-trip) pass unchanged (still on the direct path at that point); a temporary boot-time test confirmed a real write→read round-trip through the post-handoff mailbox path matches byte-for-byte; a full `service stop nvmed` → `service start nvmed` cycle with real background I/O in between (the exact scenario that used to panic) now completes cleanly and the subsequent read/write still matches byte-for-byte. Full clean regression boot with all five userspace drivers (`rtl8139d`/`hdad`/`ahcid`/`xhcid`/`nvmed`) launching successfully, all temporary diagnostics removed.
      - **What's still not done:** PCI/ACPI (one-shot enumeration work, not really in scope for this kind of migration) and GOP (the framebuffer) — GOP is now the only meaningfully-remaining, higher-stakes candidate (a bug there means no display at all).
