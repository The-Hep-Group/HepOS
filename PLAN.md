...
> **Last updated:** 2026-07-06 (scheduler `TICK_COUNT` freeze + resume-time GPF fixed this session)
...
| Scheduler (round-robin) | 🟡 | `scheduler.rs`: real preemption between the two built-in tasks (`idle`/`blink`) now works and is soak-tested (see "Known Issues" — fixed a fresh-task EOI/`sti` bug via `task_trampoline`, and a stale-segment-selector `#GP` on first genuine `iretq` resume via explicit CS/SS/DS/ES/FS/GS reload in `gdt::init()`). Still only 2 hardcoded tasks with no dynamic spawn/exit, no priorities, no blocking/wait primitives — multi-task command-worker preemption (the originally-planned use case) hasn't been re-attempted since the fix. |
...
| SATA support | 🟡 | AHCI/SATA driver exists (`ahci.rs`, raw sector read/write) but HepFS doesn't mount over it yet — still NVMe-only |
...
| 🟡 | Round-robin scheduler (naked-asm context switch) — real preemption between the 2 built-in tasks now works and is soak-tested (see Known Issues); still just 2 hardcoded tasks, no dynamic spawn/exit or blocking primitives |
...
| ✓ | AHCI/SATA driver — `ahci.rs`: PCI detect, port init, IDENTIFY, LBA48 read/write via a single polled command slot, all bounded (never `panic!`/hang on timeout). HepFS now mounts over it too via `hepfs::BlockDev` — see Storage table and Next Steps #28. |
| ✓ | virtio-gpu driver — `virtio_gpu.rs`: modern virtio-pci transport from scratch (PCI cap-list walk, virtqueue, polled), `GET_DISPLAY_INFO` + full 2D resource create/attach/scanout/transfer/flush pipeline. The real desktop now mirrors to it live (zero-copy, same backbuffer physical memory) alongside the GOP boot display — see Next Steps #18. |
...
18. ~~**virtio-gpu driver**~~ ✓ done, including display-path mirroring. `virtio_gpu.rs`: virtio-gpu has no legacy/transitional PCI ID (unlike e.g. virtio-net), so this implements the *modern* virtio-pci transport from scratch — PCI capability-list walking (`pci::config_read8`, added this session) to locate the common-config/notify-config/device-config regions, the standard virtio init handshake (ACKNOWLEDGE → DRIVER → negotiate `VIRTIO_F_VERSION_1` only → FEATURES_OK → DRIVER_OK), and one split virtqueue (controlq) driven fully synchronously. `GET_DISPLAY_INFO` plus the full `RESOURCE_CREATE_2D` → `RESOURCE_ATTACH_BACKING` → `SET_SCANOUT` → `TRANSFER_TO_HOST_2D` → `RESOURCE_FLUSH` pipeline all verified against a real QEMU device.
    - **Display-path integration** (`main.rs`, `framebuffer.rs`): the real desktop now actually renders through virtio-gpu when it's present, zero-copy — `Display::backbuf_info()` exposes the backbuffer's physical address (it was already a single `pmm::alloc_contiguous()` allocation, just not tracked) plus whether its packed pixel layout (`r_shift`/`g_shift`/`b_shift`) matches `FORMAT_B8G8R8X8_UNORM`; if so, `virtio_gpu::mirror_display()` attaches that *same* physical memory directly as a resource's backing (no separate GPU-side buffer — the CPU-side render code already writes there every frame) and sets it as scanout 0. Every full (content-dirty) redraw then calls `virtio_gpu::present_mirror()` (`TRANSFER_TO_HOST_2D` + `RESOURCE_FLUSH`) right after the existing `Display::flush()` to the GOP framebuffer — so both outputs show the same frame.
    - **Deliberately NOT touched:** HepBL's GOP framebuffer remains the actual *boot* display and the safety net — nothing here is gated on virtio-gpu succeeding; if the device or a matching pixel format isn't found, the kernel behaves exactly as before. This was a conscious scope decision (see the earlier entry above) since making the boot display itself depend on a brand-new driver initializing is a much higher-stakes change than adding a mirrored secondary output.
    - **Known limitation, not a bug:** the cheap cursor-only partial-flush path (`Display::flush_rows()`, used on every mouse move) is deliberately *not* mirrored — `present_mirror()` only does full-frame transfers today (no partial-rect support), so mirroring it on every mouse tick would turn a ~100KB partial copy into a full-frame transfer per tick. The virtio-gpu output lags cursor-only moves until the next full redraw.
    - Verified: boot-time log confirms `virtio-gpu: mirroring real backbuffer` (meaning the pixel-format check passed and the one-time resource/backing/scanout setup succeeded), zero `TRANSFER_TO_HOST_2D`/`RESOURCE_FLUSH`/timeout failures across a full boot + idle period, and all other drivers (NVMe, AHCI, HDA) unaffected. **Not independently confirmed:** actual on-screen pixel correctness on the second (virtio-gpu) display head — QEMU's SDL frontend shows one head at a time and switching to the second wasn't exercised in this test pass, so the format-match logic is protocol-verified but not eyeballed. Worth a manual check (QEMU SDL: Ctrl+Alt+2) if it matters.
...
26b. ~~**Real timer-driven preemption**~~ ✓ done — `scheduler::TICK_COUNT` freezing after the first tick (see Known Issues) is fixed: a fresh task's bare-`ret` bootstrap was orphaning the switching interrupt's EOI (`scheduler::task_trampoline` now sends it + restores `RFLAGS.IF`), and the resulting first genuine `iretq` resume was GPFing on a stale segment selector never reloaded after `lgdt` (`gdt::init()` now reloads CS/SS/DS/ES/FS/GS) — this second bug is almost certainly what the earlier preemption attempt hit and reverted over. Round-robin between `idle`/`blink` is now soak-tested stable; still just 2 hardcoded tasks, so real preemptive multitasking (ring-3 processes, background OS tasks) needs dynamic task spawn/exit next.
...
28. ~~**AHCI/SATA driver**~~ ✓ done, including HepFS integration. `ahci.rs`: detects the HBA (PCI class 0x01/0x06), maps ABAR (BAR5, always 32-bit per spec), enables AHCI mode (GHC.AE), scans the ports-implemented bitmap for the first port with `DET==3` (present + PHY established) and a plain-SATA signature (skips ATAPI/port-multiplier/enclosure), brings that port up (stop → program CLB/FB → FIS-receive-enable → start), and drives it with a single command slot (no NCQ, no interrupts — everything polled). Issues IDENTIFY DEVICE to read the real LBA48 sector count, then `read_sectors()`/`write_sectors()` build a Register H2D FIS + one PRDT entry per call. Every wait loop is bounded and *fails soft* instead of `panic!()`/hanging — directly applying the NVMe splash-screen-hang lesson from earlier this session. Wired into the QEMU launch scripts via `-device ahci,id=ahci0` + a small 64MB `hepos_sata.img` test disk (separate from the 512MB NVMe-backed `hepos_disk.img`); new `sata` terminal command reports disk size.
    - **HepFS block-device abstraction** — `hepfs::BlockDev<'a>` (`Nvme(&'a mut NvmeController)` or `Ahci`) replaces every `&mut NvmeController` parameter across all ~14 public HepFS functions plus the two private low-level `read_block`/`write_block` helpers. Turned out far less invasive than the "materially bigger refactor" this was flagged as: HepFS itself only ever touched the controller through two spots (`ctrl.lba_size`/`ctrl.lba_count` field reads, now methods; `ctrl.read_blocks`/`write_blocks` calls, kept the same method names on `BlockDev` so the call sites needed zero changes) — so the whole file's diff was one type-swap plus three one-line fixes. Across the 5 caller files (`main.rs`, `terminal.rs`, `editor.rs`, `image.rs`, `audio.rs`) there are 66 individual `hepfs::` call sites, but they route through only **11** distinct lock-acquisition points — and `terminal.rs` alone funnels 18 of those call sites through one `with_ctrl()` helper, so changing its signature (`&mut NvmeController` → `&mut hepfs::BlockDev`) fixed all 18 for free. Total: 9 small wrap-insertions (`let ctrl = &mut hepfs::BlockDev::Nvme(ctrl);`, relying on Rust's temporary-lifetime-extension so the shadowed binding keeps working with zero further changes to the calls beneath it) instead of 66 individual edits.
    - **What actually changed vs. what didn't:** the real, NVMe-mounted filesystem (boot, `edit`/`view`/`play`, HepFS window) is completely unchanged in behavior — every existing call site still explicitly passes `BlockDev::Nvme(...)`. What's new is that HepFS *can* now run over AHCI too; nothing switches the live boot filesystem to it automatically (that would be a bigger, riskier decision — migrating the one real filesystem to a different disk — not what was asked).
    - Verified with a temporary boot-time test: formatted a **fresh** HepFS directly on the separate AHCI test disk via `hepfs::BlockDev::Ahci` (entirely independent of the real NVMe filesystem, so nothing could be corrupted), then create/write/read/list-dir round-tripped correctly — alongside confirming the existing NVMe-mounted filesystem still works identically. Full clean boot after removing the test code.
    - **Bonus fixes found along the way** (unrelated to AHCI, but blocked testing): `build.sh` crashed with `qemu-img: command not found` the first time it needed to create a *new* disk image (only ever worked before because `hepos_disk.img` already existed from earlier sessions) — fixed with the same `command -v` + Program Files fallback pattern already used for `qemu-system-x86_64`. QEMU also intermittently refused to launch at all with a WASAPI recording-device error from `-device hda-duplex` — switched to `-device hda-output` (playback-only; nothing in HepOS uses microphone input, and the codec node topology `hda.rs` hardcodes is identical between the two). Both fixes applied to `build.sh` and `build.ps1`.
...
31. ~~**Two-pane file manager**~~ ✓ done, revised after feedback — first attempt built a Norton-Commander-style pair of *independent* browsers (`FilesWindow { panes: [HepfsNav; 2], active }`), which wasn't what was wanted. Reverted to a single shared `HepfsNav` per Files window and redesigned the split as tree-and-list instead: one nav bar (back/forward/path) spans the full width, then a directories-only left pane (~35% width, one-level "tree" of the current directory's subfolders) and the full dirs+files listing on the right — both reading the *same* current directory, not separate browsers. Clicking a directory in either pane navigates both (there's only one nav state to navigate). Right pane keeps file sizes and file-opening (bmp→viewer, wav→player, else→editor); left pane only ever shows directories, so it only ever navigates. Verified with a full clean boot (no automated pane-content test — layout/click-routing is straightforward geometry mirroring the already-tested single-pane code, split into a left-pane directory filter and a right-pane full listing). **Not implemented:** drag-and-drop or keyboard-shortcut copy/move between two independent locations — that's a materially different feature (needs two independent navigable locations, which this design deliberately doesn't have) and wasn't asked for.
32. ~~**Window animations**~~ ✓ done — 180ms ease-out scale (centered on the window's own position) on open (creation + unminimize) and close/minimize. `Window::show()`/`hide()` replace direct `minimized` field writes at all 11 call sites; a `hide()`'d window keeps `minimized == false` and keeps rendering (at a shrinking scale, via `eased_rect()`) until `Desktop::tick_anims()` sees the animation finish and *then* sets `minimized = true` — that's what let the close animation reuse the existing render loop without a parallel "closing windows" list. Content renderers needed zero changes since they already accept arbitrary `(wx, wy, ww, wh)` and adapt (same code path as live window resize). `draw_window()` now takes an explicit rect instead of reading `win.x/y/w/h` so chrome tracks the eased geometry too. Boot-time windows (ids 0-7) skip the animation the very first time only, because `TSC_PER_MS` isn't calibrated yet when they're created (harmless self-heals — every animation after boot times correctly). Verified with a temporary boot-time test: Opening starts smaller and grows to full size, Closing stays un-minimized+rendering until its animation completes, both confirmed via forced-elapsed `tick_anims()` calls.
    ...
    - **Second follow-up round (user testing surfaced more issues):**
      - **Bug: minimize "just closed" the window instead of running in the background.** `grouped_taskbar_entries()` filtered on `!w.minimized`, so a window that had been minimized (as opposed to never-yet-opened) dropped off the taskbar entirely — indistinguishable from actually being gone. Added `Window::ever_shown: bool` (true from creation/`show()`, false only for boot-time `hide_instant()` windows that were never actually shown) and switched the taskbar filter to that instead — a minimized-but-previously-open window now keeps its taskbar button (dimmed, since nothing of that kind is currently visible), same as any normal OS's minimize. Clicking that button now `show()`s + focuses it instead of assuming it must already be open. Also found and fixed 3 more raw `w.minimized = false` writes (`editor.rs`, `image.rs`, `terminal.rs`'s `edit`/`view`/`play` open paths) that had been missed when the rest of the codebase was converted to `show()`/`hide()` earlier — those would've skipped both the animation and the new `ever_shown` bookkeeping.
      - **Bug: dragging a maximized window's title bar did nothing** (couldn't un-maximize by dragging, and thus couldn't drag it back to the top edge to re-maximize either — same root cause). The entire drag-start code path was gated behind `else if !win.maximized`, so a maximized window's title bar wasn't draggable at all. Fixed: dragging a maximized window's title bar now restores it first (`toggle_maximize()`, which computes the click's fractional position along the old wide title bar so the restored window reappears roughly under the cursor) and immediately starts a normal drag — which naturally flows into the existing snap-zone logic, so dragging back to the top re-maximizes it, same as any real OS.
      - **Start Menu now animates open** — 150ms ease-out slide up from behind the taskbar (`Desktop::start_menu_anim_tsc`, `start_menu_progress()`/`start_menu_animating()`), closing stays instant. Hit-testing always uses the final target position, not the animated one, since a 150ms slide isn't something you can click precisely mid-flight anyway.
      - **Open/close animation redesigned to read as "to/from the taskbar."** The original version just scaled 70%↔100% in place; now it also slides vertically by up to the window's own height (down for closing, up for opening) while shrinking further (down to ~10% instead of 70%), so minimizing/restoring visibly reads as "sinking into"/"rising from" the taskbar rather than a static in-place fade.
      - Chrome-animation polish beyond this (e.g. Start Menu *closing*, taskbar popups) intentionally still not done — scoped to what was explicitly asked for.
    - **Third follow-up round:**
      - **Start Menu now animates closing too**, not just opening. Same shape as the window close fix in the first round: `start_menu_open` still flips to `false` immediately (every other check — input routing, hit-testing — needs to treat the menu as closed right away), but `Desktop::close_start_menu()` also starts a `MenuAnimKind::Closing` animation, and `draw_start_menu()` keeps rendering (sliding back down behind the taskbar over 150ms) as long as that's in flight, mirroring `Window::is_closing()`/`eased_rect()`'s pattern. `open_start_menu()`/`close_start_menu()` replace all 6 raw `start_menu_open = …` writes so every code path (Start button toggle, picking a program, clicking a taskbar button, right-click background menu, clicking the desktop) gets the animation consistently instead of only the one call site that happened to set it directly.
    - **Fourth follow-up round:**
      - **Bug: close (✕) and minimize (_) were identical — both kept the window in the taskbar.** Added `Window::close()`, which does the same shrink animation as `hide()` (used for minimize) but also sets `ever_shown = false` immediately, dropping the taskbar entry right away (same field `hide_instant()` already used to keep never-opened boot windows off the taskbar). `show()` already sets `ever_shown = true`, so reopening a previously-closed window (editor `edit`, image viewer, HepFS click, etc.) correctly brings its taskbar entry back. Close-button clicks and the editor's own Ctrl+Q/ESC/F10 close path (in `main.rs`, both the main Editor window and `EXTRA_EDITORS`) now call `close()`; the "_" minimize button and clicking a focused taskbar button (which is a genuine minimize, not a close) still call `hide()`.
      - **Investigated the "freeze at Tianocore splash" / "display not init" reports.** Root cause wasn't a HepOS bug — it was a leftover `qemu-system-x86_64` process from this session's own rapid build/boot/kill test cycles that `pkill` (run from Git Bash) silently failed to actually terminate, left running in the background holding a lock on `hepbl_vars.fd` (OVMF's NVRAM store, reused across runs by `build.sh`/`build.ps1` rather than recreated each time). A stray process holding that file can collide with a fresh QEMU launch trying to use the same NVRAM image, which is consistent with intermittent/unexplained firmware-stage hangs. Killed the stray process (confirmed via `Get-Process`/`Stop-Process`, since `pkill` from Git Bash doesn't reliably signal native Windows processes) and deleted the (possibly torn, from earlier hard-kills) `hepbl_vars.fd` so `build.sh` regenerates a clean copy from the OVMF template on the next run. If this recurs: check `Get-Process qemu-system-x86_64` for stragglers before assuming it's a code bug, and prefer a graceful QEMU shutdown (or `Stop-Process`) over `pkill`/closing the window during testing.
