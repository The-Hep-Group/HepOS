...
| Custom filesystem supporting SATA **and** NVMe | 🟡 | HepFS still only mounts over **NVMe**. An AHCI/SATA driver now exists (`ahci.rs`, verified raw read/write) but HepFS isn't wired to it yet — it's block-device-agnostic in name only; every call site talks to `nvme::` directly. |
...
| 🟡 | AHCI/SATA driver — `ahci.rs`: PCI detect, port init, IDENTIFY, LBA48 read/write via a single polled command slot, all bounded (never `panic!`/hang on timeout). Verified with a boot-time byte-exact write/read round-trip against a real QEMU AHCI disk. **Not yet wired into HepFS** (still NVMe-only) — see Storage table and Next Steps #28. |
...
| 🟡 | SATA support — `ahci.rs` driver exists and is verified (raw sector read/write); HepFS itself still only mounts over NVMe, so nothing actually stores data on the SATA disk yet |
...
28. 🟡 **AHCI/SATA driver** — the driver itself is done and verified; **HepFS still doesn't mount over it** (that's the remaining, larger part of this item). `ahci.rs`: detects the HBA (PCI class 0x01/0x06), maps ABAR (BAR5, always 32-bit per spec), enables AHCI mode (GHC.AE), scans the ports-implemented bitmap for the first port with `DET==3` (present + PHY established) and a plain-SATA signature (skips ATAPI/port-multiplier/enclosure), brings that port up (stop → program CLB/FB → FIS-receive-enable → start), and drives it with a single command slot (no NCQ, no interrupts — everything polled). Issues IDENTIFY DEVICE to read the real LBA48 sector count, then `read_sectors()`/`write_sectors()` build a Register H2D FIS + one PRDT entry per call (one contiguous physical buffer; no scatter-gather yet). Every wait loop is bounded and *fails soft* (logs + returns `false`) instead of `panic!()`-ing or spinning forever — directly applying the lesson from the NVMe splash-screen-hang fix earlier this session, since an under-budgeted or unbounded polling loop during boot-time driver init is exactly the bug class that freezes the whole OS before a single frame ever renders. Wired into the QEMU launch scripts via an explicit `-device ahci,id=ahci0` HBA + a small 64MB `hepos_sata.img` test disk (kept separate from the 512MB NVMe-backed `hepos_disk.img`). New `sata` terminal command reports disk size. Verified with a temporary boot-time test: wrote a 512-byte pattern to sector 100, read it back, byte-exact match — then removed. **Remaining work** (tracked here, not yet started): give HepFS a block-device abstraction (today every call site — `hepfs.rs`, `main.rs`, `terminal.rs`, `editor.rs`, `image.rs`, `audio.rs` — talks to `nvme::` directly by name) so it can mount over either backend; that's a materially bigger refactor than the driver itself.
