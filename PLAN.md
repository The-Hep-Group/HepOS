...
- **Still a follow-up, not done here**: the large-icon-grid visual reskin (currently a two-pane tree+list, now with real per-type icons — see above) and mouse-wheel scrolling sized to the window; no rubber-band/marquee select in the file manager (Shift+click range-select only) — see Next Steps.
...
9. **Real desktop shell, remaining pieces** — desktop icons are real/draggable(grid-snapped)/renamable/file-backed and actually open on double-click (files by type, directories as a new Files window); taskbar+Start-Menu pin/unpin (+ taskbar drag-to-reorder, pinned/minimized/running now visually distinct), real pixel-art icon glyphs everywhere (not flat colors), file-manager select/rename-matching-desktop, drag-and-drop between/within Files windows (with real drag-onto-".." → move to parent, visual drag feedback, and real Shift+click multi-select drag/copy — not just highlighting), a left-edge pin dock for files/directories, and right-click "Open/Pin/Copy"/"Paste" menus on both the desktop and the file manager are all done (see "Desktop Icons"/"Left Pin Dock"/"Taskbar & Start Menu"/"HepFS File Manager" above). What's left of that same original request:
    ...
    - No rubber-band/marquee select in the file manager (unlike the desktop — see "Desktop Icons" above) — only Shift+click range-select.
