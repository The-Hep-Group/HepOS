...
- **Double-clicking a file-backed desktop icon now actually opens it**: `Desktop::open_fs_entry_requested: Option<(ino, is_dir, name)>` — set by `open_icon()`, since desktop.rs has no access to the block device or editor/image/audio state; `main.rs` polls it once per frame (same `*_requested` pattern as `new_window_requested`/`open_settings_requested`) and either spawns a new Files window navigated straight to that directory (`nav.ino`/`nav.path` set directly, skipping the normal click-to-navigate path) or dispatches the file by extension exactly like the file manager already does (`.bmp`→image viewer, `.wav`→audio player, else→editor).
...
- **Shift+click range-select**: highlights every row between the last plain-clicked row and a Shift+clicked one, within the same pane (`HepfsNav::range_selected`) — see Known Issues for exactly what this does and doesn't do (highlight-only, not multi-move).
...
- **Copy/paste with Ctrl+C/Ctrl+V** (files-and-directories, separate from the plain-text clipboard editor/terminal Ctrl+C/V already used — `clipboard.rs`): Ctrl+C in a focused Files window copies whichever row is `nav.selected` into `main.rs`'s `FS_CLIPBOARD` static (`(parent_ino, ino, name, is_dir)`, resolved via a new `selected_fs_entry()` helper — the same pane/row→entry traversal the click handler already did, just driven by the *last selected* row instead of a live click); Ctrl+V (or plain Ctrl+V — 0x16 — same as the terminal's own binding) pastes it into whichever Files window has focus, via `hepfs::copy_entry()` (new): unlike `move_entry()`, this makes a genuine duplicate — new inode, new data blocks, recursing into subdirectories — not just a re-pointed `DirEntry`. A name collision at the destination (e.g. pasting into the same directory it was copied from) is retried as `"<name> (1)"`, then `"(2)"`, and so on (capped at 100 attempts) until a free name is found — matches the numbered-duplicate convention rather than a single fixed `"(copy)"` suffix.
...
- **Still a follow-up, not done here**: the large-icon-grid visual reskin (currently a two-pane tree+list, now with real per-type icons — see above) and mouse-wheel scrolling sized to the window; Shift+click *range-select* is done (above), but it's highlight-only — dragging still only ever moves the one row the drag started on, unlike the desktop's Shift+click/marquee (see "Desktop Icons" above), which moves every selected icon together — see Next Steps.
...
| ~~File manager had no Shift+click range-select~~ | Added. Shift+click on a row selects every row between the current anchor (`nav.selected`) and the clicked one, within the same pane — highlight-only (`HepfsNav::range_selected: Vec<usize>`), doesn't move the anchor and doesn't arm a drag/open/rename, mirroring how the desktop's own Shift+click is a pure selection modifier. Unlike the desktop, this doesn't extend to multi-*move* — the file manager still only drags one row at a time; see Next Steps. Cleared on any navigation (back/forward/`..`/into a directory) so a stale range from a different folder can't linger. Verified with a temporary test replicating the exact anchor+click→range formula the click handler uses. |
...
9. **Real desktop shell, remaining pieces** — desktop icons are real/draggable(grid-snapped)/renamable/file-backed and actually open on double-click (files by type, directories as a new Files window); taskbar+Start-Menu pin/unpin (+ taskbar drag-to-reorder, pinned/minimized/running now visually distinct), real pixel-art icon glyphs everywhere (not flat colors), file-manager select/rename-matching-desktop, and drag-and-drop between/within Files windows are all done (see "Desktop Icons"/"Taskbar & Start Menu"/"HepFS File Manager" above). What's left of that same original request:
    - A left-edge pinned dock, separate from the bottom taskbar — this is a *new UI element* (nothing like it exists today) for dragging files/directories onto to pin them, positioned so it doesn't collide with the desktop icon column at `ICON_X=16`. Distinct from taskbar pinning (done) and desktop icon pinning (done) — this would be a third place to pin things.
    - The file manager's large-icon-grid visual reskin + mouse-wheel scrolling sized to the window — it's still a two-pane tree+list (now with real per-type icons on each row and drag-and-drop, see "HepFS File Manager" above), not an icon grid.
    - Shift+click/rubber-band multi-select (drag moves the whole selected group) was added to the desktop (see "Desktop Icons" above); the file manager only got Shift+click *range-select highlighting* (`HepfsNav::range_selected`) — dragging still only ever moves the single row the drag started on, and there's no marquee/rubber-band select in the file manager at all.
