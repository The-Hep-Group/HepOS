...
- Verified with a temporary boot-time test: create 2 entries → icon count grows by 2; rename one → old label gone, new label present; direct position mutation confirmed the same fields `update_mouse`'s drag path writes actually persist. Full boot regression clean.
- **What's NOT done yet** (this was one slice of a much larger request — see Next Steps): double-clicking an `FsEntry` icon doesn't yet actually open anything (only `Program` icons do — opening a file by its type or a directory as a Files window needs main.rs's editor/image/audio dispatch, not just desktop.rs); no Start Menu "Add to Desktop"; no drag-and-drop between multiple Files windows; no left-edge pinned dock; the file manager itself still opens on a single click (no select-then-click-again-to-rename there yet, unlike the desktop).
...
9. **Real desktop shell, remaining pieces** — desktop icons are now real/draggable/renamable and file-backed (see "Desktop Icons" above); these are the parts of that same request not yet done:
    ...
    - Start Menu right-click → "Add to Desktop" for a program (no per-entry context menu on Start Menu rows exists yet at all).
    - Drag-and-drop of files between multiple open Files windows.
    - A left-edge pinned dock, separate from the bottom taskbar, that files/directories/programs can be dragged onto to pin (this is an entirely new UI element — nothing like it exists today; would need to pick a width that doesn't collide with the desktop icon column at `ICON_X=16`).
    - File manager: single click to select (currently single-click opens immediately, same old pattern the desktop icons used to have); click-again-to-rename, matching what the desktop now does.
