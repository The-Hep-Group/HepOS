...
struct Mailbox {
    ...
    /// (`service stop ahcid` / `kill <pid>`); see `stop_service()`'s doc
...
// enable/disable is in-memory only for this session). AHCI additionally
// serializes against `AHCI_IO_LOCK` since a stop/start shouldn't race an
// in-flight `read_sectors()`/`write_sectors()` transaction.
static ENABLED: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(true);
pub const SERVICE_NAME: &str = "<ahcid>";
...
pub fn stop_service() -> Result<(), &'static str> {
    ...
    drop(_io); // release before the wait — a stray in-flight I/O call would otherwise deadlock against it
...
pub fn start_service() -> Result<(), &'static str> {
    ...
    let result = (|| {
        let (mb, mailbox_phys) = {
            ...
            match g.as_ref() {
                Some(c) => match (c.mailbox, c.mailbox_phys) {
                    (Some(mb), Some(p)) => (mb, p),
                    _ => return Err("driver not initialized"),
                },
                None => return Err("driver not initialized"),
            }
        };
        if AHCID_ELF.is_empty() { return Err("ahcid ELF not built"); }
        unsafe { core::ptr::write_volatile(&mut (*mb).stop, 0); }
        process::exec_async_with_arg(usize::MAX, SERVICE_NAME, AHCID_ELF, mailbox_phys)?;
        for _ in 0..STOP_WAIT_SPINS {
            if is_running() { return Ok(()); }
            core::hint::spin_loop();
        }
...
pub fn init(devs: &[pci::PciDevice]) -> bool {
    ...
    if let Some(mailbox_phys) = pmm::alloc_contiguous(MAILBOX_PAGES) {
        ...
        ctrl.mailbox_phys = Some(mailbox_phys);
