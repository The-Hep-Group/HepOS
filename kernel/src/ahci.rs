...
pub fn init(devs: &[pci::PciDevice]) -> bool {
    ...
    let mut ctrl = AhciController {
        port: AhciPort { abar, port_base, ctba_virt: vmm::phys_to_virt(ctba_phys) },
        sectors: 0,
        sector_size: 512,
    };
...
/// Read `count` sectors (max ~8M sectors per call — one PRDT entry, 22-bit
/// byte count field) starting at `lba` into the contiguous physical buffer
/// `buf_phys`. Returns false on any failure (no disk, timeout, task-file error).
pub fn read_sectors(lba: u64, count: u16, buf_phys: u64) -> bool {
    let guard = CONTROLLER.lock();
    let Some(ctrl) = guard.as_ref() else { return false; };
    let fis = lba_fis(ATA_CMD_READ_DMA_EXT, lba, count);
    issue_command(ctrl, fis, Some((buf_phys, count as u32 * ctrl.sector_size)), false)
}
...
pub fn write_sectors(lba: u64, count: u16, buf_phys: u64) -> bool {
    let guard = CONTROLLER.lock();
    let Some(ctrl) = guard.as_ref() else { return false; };
    let fis = lba_fis(ATA_CMD_WRITE_DMA_EXT, lba, count);
    issue_command(ctrl, fis, Some((buf_phys, count as u32 * ctrl.sector_size)), true)
}
