...
/// Run the hwtest ELF built from userspace/hwtest — a proof-of-concept
/// userspace "driver" that reads real hardware (RTC seconds via port I/O,
/// then the Local APIC ID via MMIO) entirely through SYS_PORT_IN/OUT and
/// SYS_MMAP_MMIO, never touching kernel code paths directly. See PLAN.md's
/// "Userspace drivers" writeup for what this does and doesn't prove.
pub fn run_hwtest() -> Result<u64, &'static str> {
    if HWTEST_ELF.is_empty() {
        return Err("hwtest ELF not built — run build.ps1 first");
    }
    exec("<hwtest>", HWTEST_ELF)
}
